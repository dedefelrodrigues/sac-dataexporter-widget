(function () {
  console.log("[DataExporter v6] Script starting...");
  ...
  class DataExporter extends HTMLElement {
    constructor() {
      ...
      console.log("[DataExporter v6] Constructor done.");
    }
    ...
    _handleExport() {
      ...
      if (!this.dataBinding || !this.dataBinding.data) {
        statusLine.textContent = "No data linked. Please link a table to this widget first.";
      ...
      if (!data || data.length === 0) {
        statusLine.textContent = "No data rows to export.";
        statusLine.className = "status-line visible error";
        this.dispatchEvent(new Event("onExportError"));
        return;
      }

      statusLine.textContent = "Generating CSV...";
      ...
      // Trigger download
      _downloadFile(this, csvContent);

      var rowCount = data.length;
      statusLine.textContent = "Exported " + rowCount + " row" + (rowCount !== 1 ? "s" : "") + ".";
  ...
  // ── Download (module-scoped) ─────────────────────────────────────────
  function _downloadFile(widget, csv) {
  ...
  // ── Register ──────────────────────────────────────────────────────
  console.log("[DataExporter v6] Registering custom element...");
  customElements.define("com-sac-dataexporter", DataExporter);
  console.log("[DataExporter v6] Custom element registered successfully.");
})();
